...
func (m *Milvus) Upsert(ctx context.Context, vendor string, documents []*api.Document) error {
	var idList []string
	var textList []string
...
func (m *Milvus) Query(ctx context.Context, vendor string, vector []float64, topK int, minScore float64) ([]*api.Similarity, error) {
	...
	if len(result) == 0 {
		return []*api.Similarity{}, nil // ensure `[]` in JSON if empty
	}
...
func (m *Milvus) Delete(ctx context.Context, vendor string, sourceIDs ...string) error {
...
func constructSimilaritiesFromResult(result *client.SearchResult, minScore float64) ([]*api.Similarity, error) {
	...
	similarities := make([]*api.Similarity, 0) // ensure `[]` in JSON if empty
